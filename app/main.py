...
def recommend_movies(payload: RecommendationRequest, session: Session = Depends(db_session)):
    if not payload.selected_movie_ids:
        raise HTTPException(status_code=400, detail="selectedMovieIds is required")
    ...
    selected_query = (
        select(
            ...
            Movie.runtime_minutes.label("runtime_minutes"),
            func.coalesce(
                select(func.group_concat(MovieGenre.genre, ",")).where(MovieGenre.movie_id == Movie.id).scalar_subquery(),
                literal(""),
            ).label("genres"),
            MovieRating.average_rating.label("average_rating"),
            MovieRating.num_votes.label("num_votes"),
        ...
        .select_from(Movie)
        .outerjoin(MovieRating, MovieRating.movie_id == Movie.id)
        .where(Movie.id.in_(selected_ids))
    )
    selected_movies = session.execute(selected_query).mappings().all()
    if len(selected_movies) != len(selected_ids):
        raise HTTPException(status_code=404, detail="One or more selected movie ids were not found")
    ...
    if selected_genres:
        candidate_filters.append(
            exists(
                select(1)
                .select_from(MovieGenre)
                .where(MovieGenre.movie_id == Movie.id, MovieGenre.genre.in_(sorted(selected_genres)))
            )
        )
    if selected_people:
        candidate_filters.append(
            exists(
                select(1)
                .select_from(MoviePrincipal)
                .where(MoviePrincipal.movie_id == Movie.id, MoviePrincipal.person_id.in_(sorted(selected_people)))
            )
        )
    if selected_types:
        candidate_filters.append(Movie.title_type.in_(sorted(selected_types)))
    ...
    principal_people = {
        row["movie_id"]: {row["person_id"] for row in session.execute(select(MoviePrincipal.person_id).where(MoviePrincipal.movie_id == row["movie_id"])).scalars().all()}
        for row in candidate_rows
    }
    crew_people = {
        row["movie_id"]: {row["person_id"] for row in session.execute(select(MovieCrewLink.person_id).where(MovieCrewLink.movie_id == row["movie_id"])).scalars().all()}
        for row in candidate_rows
    }
    ...
    for row in candidate_rows:
        ...
        recommendation_score = (
            ...
            + shared_people * 2.5
            + shared_crew * 2.0
            + shared_title_types * 0.5
            + (row["average_rating"] or 0) / 10.0
            + (row["num_votes"] or 0) / 1000.0
        )
        scored.append(
            RecommendationItem(
                id=movie_id,
